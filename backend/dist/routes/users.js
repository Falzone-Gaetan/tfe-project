...
const userRoutes = (connection, bcrypt) => {
    ...
    router.post('/register', async (req, res) => {
        try {
            ...
            const [existingUser] = await connection.execute('SELECT * FROM users WHERE name = ?', [name]);
            ...
            await connection.execute('INSERT INTO users (name, password, email) VALUES (?, ?, ?)', [name, hashedPassword, email]);
            return res
                .status(201)
    ...
    router.put('/password', async (req, res) => {
        try {
            ...
            // Récupérer l'utilisateur depuis la base de données
            const [rows] = await connection.execute('SELECT * FROM users WHERE name = ?', [username]);
            const user = rows[0];
            // Vérifier si l'utilisateur existe
            if (!user) {
                return res.status(404).json({ message: 'Utilisateur non trouvé' });
